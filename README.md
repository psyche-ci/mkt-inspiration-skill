...
你制作的 HTML 页面已随 Skill 一起上传到 [`app/`](./app/)：包含页面 HTML、前端数据脚本、交互脚本和本地 AI 代理启动脚本。别人安装 Skill 后使用这套应用，会沿用相同的页面版式、卡片结构和 AI 读创意流程；抓取新内容时仍按同一模板渲染。API Key 需要每位使用者在自己的浏览器中单独设置，仓库不包含真实 Key。
