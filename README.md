...
网页应用已经把你制作的版式随 Skill 一起打包：

- 首页：MKT INSPIRATION 九宫格，保留页眉、筛选、卡片、封面、AI 帮你读和收藏交互。
- 灵感库首页：四个真实文件夹视觉入口——内容向、形式向、节点向、视觉向。
- 文件夹详情页：左侧竖向分类栏、标题区、返回上一级/上传创意、真实封面、创意洞察/内容/形式、笔记和关联操作。
- 资源：页面字体、文件夹图和详情卡片封面均随 `app/public/` 提供，避免安装后只剩页面骨架或黑屏占位。

安装后，抓取结果会沿用这套页面和卡片结构；API Key 仍由每位使用者在自己的浏览器本地设置，不会写入 Skill。
